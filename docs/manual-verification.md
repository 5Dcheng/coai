...
验证 `CoAI: Sync mapper from git changes` 能根据 git 变更与 `@coai anchor` 重新定位 mapper 行号。

推荐测试方式：

1. 准备一个独立 git 工作区，并确保 `.coai/project`、`.coai/mapper`、目标代码文件都在同一个仓库内。
2. 在代码文件中插入或删除若干空行，让某个 `@coai anchor` 的真实行号变化。
3. 保持对应 `.mapper.json` 中的旧 `line` 不变。
4. 在 Extension Development Host 中打开该 git 工作区。
5. 执行命令 `CoAI: Sync mapper from git changes`。
6. 打开 `Output -> CoAI Navigation` 查看同步明细。
7. 确认对应 mapper 项的 `line` 与 `lastUpdated` 已更新。
8. 回到功能文档，执行 Ctrl/Cmd + Click，确认能跳到正确代码位置。
...
验证 `CoAI: Run pre-commit CoAI check` 会先执行 git-sync，再根据 open bug 状态给出提交前提示。

步骤：

1. 在当前工作区制造一个会被 git-sync 识别的问题，或保留一个现有 open bug。
2. 执行命令 `CoAI: Run pre-commit CoAI check`。
3. 打开 `Output -> CoAI Navigation` 查看执行明细。
4. 若存在 bug，验证命令提示可直接打开第一个 bug 日志。
5. 若没有 bug，验证命令提示“提交前检查通过，可继续提交”。

预期结果：

* Output 中先出现 git-sync 明细
...
3. 观察终端输出是否包含 git-sync 明细与 `[PreCommit] pass` / `pass with warnings`。
